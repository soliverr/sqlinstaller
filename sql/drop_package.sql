...
-- Drop package
--
-- Usage: drop_package PACKAGE_NAME [SCHEMA]
--
--
...
begin
  ...
  select count(1) into l$cnt
   ...
   if l$cnt > 0 then
     execute immediate 'drop package ' || l$schema || '.' || '&1';
     dbms_output.put_line( CHR(10) || 'I: Package ' || l$schema || '.' || upper('&1') || ' droped' );
   else
     dbms_output.put_line( CHR(10) || 'W: Package ' || l$schema || '.' || upper('&1') || ' does not exists' );
   end if;
end;
/
